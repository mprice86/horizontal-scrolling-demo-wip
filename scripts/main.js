/**
 * Parallactic Framework by Matthew Price 2019
 */
var parallactic = (function () {
  ...
  var parallaxPosition = (function () {
    var anchorElements = document.querySelectorAll('*[para-anchor]');
    var coordinateElements = document.querySelectorAll('*[para-coords]');
    ...
    if (coordinateElements.length) {
      initCoordinateElements();
    }

    function initCoordinateElements() {
      for (var i = 0; i < coordinateElements.length; i++) {
        var positionString = coordinateElements[i].getAttribute('para-coords');
        var positionArray = positionString.replace(/ /g, '').split(',');

        if (positionArray.length !== 4) {
          console.log('Parallax Coordinates not valid.');
          return;
        }

        var position = {
          top: positionArray[0],
          right: positionArray[1],
          bottom: positionArray[2],
          left: positionArray[3],
        }

        console.log(position);
        var top, right, bottom, left;

        top = position.top !== 'auto' ? position.top + '%' : position.top;
        right = position.right !== 'auto' ? position.right + '%' : position.right;
        bottom = position.bottom !== 'auto' ? position.bottom + '%' : position.bottom;
        left = position.left !== 'auto' ? position.left + '%' : position.left;

        var styleString = `position: absolute; top: ${top}; right: ${right}; bottom: ${bottom}; left: ${left};`

        console.log(styleString);

        coordinateElements[i].setAttribute('style', styleString);
      }
    }

    function initAnchorElements() {
      for (var i = 0; i < anchorElements.length; i++) {
        var target = document.getElementById(anchorElements[i].getAttribute('para-anchor'));
        var position = anchorElements[i].getAttribute('para-position');
        ...
        positionElement(target, anchorElements[i], position);
      }
    }
